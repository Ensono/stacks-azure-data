...
resource "null_resource" "approve_adf_blob_private_endpoint" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
        az login --service-principal -u ${azurerm_client_config.current.client_id} -p ${azurerm_client_config.current.client_secret} --tenant ${azurerm_client_config.current.tenant_id}
