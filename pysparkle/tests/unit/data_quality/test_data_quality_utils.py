...
def test_create_datasource_context():
    context = create_datasource_context(TEST_DQ_CONF["dataset_name"], TEST_DQ_CONF["gx_directory_path"])
    assert context.list_datasources()[0]["name"] == TEST_DQ_CONF["dataset_name"]
    assert (
        list(context.list_datasources()[0]["data_connectors"].keys())[0]
        == f'{TEST_DQ_CONF["dataset_name"]}_data_connector'
    )
