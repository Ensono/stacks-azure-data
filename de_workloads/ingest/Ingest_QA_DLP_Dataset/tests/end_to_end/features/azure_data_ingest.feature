...
Scenario Outline: Data Factory Ingest SQL Database into ADLS with Data Quality
    Given the ADF pipeline Ingest_QA_DLP_Dataset_DQ has been triggered with <parameters>
    And I poll the pipeline every 10 seconds until it has completed
    And the ADF pipeline Ingest_QA_DLP_Dataset_DQ has finished with state Succeeded
    ...
    |{"window_start" : "2010-01-01", "window_end": "2010-01-31"}|["SalesLT.Customer", "SalesLT.Product", "SalesLT.Product_dq", "SalesLT.ProductModel", "movies.ratings_small"]|
